...
def style_plotly(fig, theme: str, title: str | None = None, subtitle: str | None = None):
    ...
    Legend is placed below plot to avoid subtitle overlap.
    """
    if theme == "dark":
    ...
    fig.update_xaxes(showgrid=True, gridcolor=grid_color, zeroline=False, showline=False, linecolor=axis_line, ticks="outside")
    fig.update_yaxes(showgrid=False, zeroline=False, showline=False, linecolor=axis_line, ticks="outside")
...
def style_table(df: pd.DataFrame, theme: str):
    """Force dataframe colors to match theme (Streamlit grid can ignore global CSS)."""
    if theme == "dark":
        bg = "#0b0b0f"
        panel = "rgba(255,255,255,0.06)"
        border = "rgba(255,255,255,0.10)"
